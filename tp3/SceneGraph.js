    ...
    */
    ...
    createMaterials(materials) {
        ...
        for (var key in materials) {
            ...
            if(bumpTexture) {
                ...
                this.materialsDict[material.id].bumpScale = material.bumpscale;
            }
    ...
    traverseNode(node) {
        if(node.type === "node") {
            ...
            this.nodesDict[node.id].name = node.id;
    ...
    pushTransformations(node) {
        ...
        for (let transformation of node.transformations) {
            ...
            if (transformation.type === 'T') {
                ...
                const rotationX = transformation.rotation[0] *Math.PI/180;
                const rotationY = transformation.rotation[1] *Math.PI/180;
                const rotationZ = transformation.rotation[2] *Math.PI/180;
    ...
    pushMaterial(materialId) {
        let material = this.materialsDict[materialId];
        if(!material) {
            this.materialStack.push(this.defaultMaterial);
        }
        else {
